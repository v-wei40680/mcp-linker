...
export default function OnBoardingPage() {
  ...
  return (
    <div className="p-4 space-y-4">
      {isVerifyingToken && <div className="text-gray-500">Verifying token...</div>}
      ...
      {!isVerifyingToken && (
        <>
          <div className="text-sm text-gray-700">
            <strong>Access Token:</strong> {access_token ? "Available" : "Not Found"}
          </div>

          {me?.user_metadata.user_name ? (
            <div className="space-y-2">
              <div className="text-lg font-semibold">Welcome to MCP-Linker</div>
              <div><strong>Username:</strong> {me.user_metadata.user_name}</div>
              <div><strong>Email:</strong> {me.email}</div>
              <div><strong>User ID:</strong> {me.id}</div>
            </div>
          ) : (
            <form
              onSubmit={(e) => {
